...
class tflStreamWrapper {
  ...
  function stream_write($data) { // caled when data written by cURL
    ...
    $insert = "INSERT INTO stop_reference (stoppointname,stopid,stopcode1,stopcode2,stoppointtype,towards,bearing,stoppointindicator,stoppointstate,latitude,longitude) ";

    $delete = "DELETE FROM stop_reference (stoppointname,stopid,stopcode1,stopcode2,stoppointtype,towards,bearing,stoppointindicator,stoppointstate,latitude,longitude) WHERE stopid=";
    ...
    for ($i=0; $i < count($stop_data); $i++) {
	  ...
	  // Set up string to insert values
          $sql = $insert."values ($entry[1],$entry[2],$entry[3],$entry[4],$entry[5],$entry[6],$entry[7],$entry[8],$entry[9],$entry[10],$entry[11])";

	  // Set up string to remove any existing data for the StopID
	  $delete_entry = $delete."$entry[2]";
	  
	  try {
	    $DEL = $GLOBALS['DBH']->prepare($delete_entry);
	    $DEL->execute();
...
// Set cURL options:
curl_setopt($curl, CURLOPT_URL, "http://countdown.api.tfl.gov.uk/interfaces/ura/stream_V1?StopAlso=true&ReturnList=StopPointName,StopID,StopCode1,StopCode2,StopPointType,Towards,Bearing,StopPointIndicator,StopPointState,Latitude,Longitude");
